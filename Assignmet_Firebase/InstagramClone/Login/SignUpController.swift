...
class SignUpController: UIViewController, UINavigationControllerDelegate {
    ...
    private let signUpButton: UIButton = {
        ...
        button.addTarget(SignUpController.self, action: #selector(handleSignUp), for: .touchUpInside)
    ...
    private let alreadyHaveAccountButton: UIButton = {
        ...
        button.addTarget(SignUpController.self, action: #selector(handleAlreadyHaveAccount), for: .touchUpInside)
        return button
    }()
    ...
    private func setupInputFields() {
        ...
        stackView.anchor(top: view.topAnchor, left: view.safeAreaLayoutGuide.leftAnchor, right: view.safeAreaLayoutGuide.rightAnchor, paddingTop: 20, paddingLeft: 40, paddingRight: 40, height: 200)
    }
    ...
    @objc private func handleSignUp() {
        ...
        AuthManager.shared.checkIfUserExist(phoneNumber: phoneNumber) { documentSnapshot, error in
            if let error = error {
                ...
                showAlert(title: "User Exists Pleaase login", message: "", viewController: self)
            } else {
    ...
    func signUp(phoneNumber: String) {
        ...
        PhoneAuthProvider.provider().verifyPhoneNumber("+91\(phoneNumber)", uiDelegate: nil) { [self] verificationID, error in
            ...
            signUpButton.setTitle("Verify", for: .normal)
            signUpButton.setTitle("Verify", for: .selected)
            if !codeTextField.isHidden {
        ...
        Auth.auth().signIn(with: credential) { authResult, error in
            ...
            changeRequest.commitChanges { error in
                if let error = error {
                    ...
                    showAlert(title: "Failed To Create user", message: "", viewController: self)
                } else {
