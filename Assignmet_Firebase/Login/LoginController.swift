...
class LoginController: UIViewController {
    ...
    private let logoContainerView: UIView = {
        ...
        title.anchor(width: 200, height: 100)
    ...
    private let loginButton: UIButton = {
        ...
        button.setTitle(Const.login, for: .normal)
    ...
    private let dontHaveAccountButton: UIButton = {
        let button = UIButton(type: .system)
        let attributedTitle = NSMutableAttributedString(string: Const.donNottHaveAnAccount, attributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 14), NSAttributedString.Key.foregroundColor: UIColor.lightGray])
        attributedTitle.append(NSAttributedString(string: Const.signUP, attributes: [NSAttributedString.Key.font: UIFont.boldSystemFont(ofSize: 14), NSAttributedString.Key.foregroundColor: UIColor.mainBlue
            ]))
    ...
    private func setupInputFields() {
        ...
        stackView.anchor(top: logoContainerView.bottomAnchor, left: view.leftAnchor, right: view.rightAnchor, paddingTop: 40, paddingLeft: 40, paddingRight: 40)
    }
    ...
    @objc private func handleLogin() {
        ...
        AuthManager.shared.checkIfUserExist(phoneNumber: phoneNumber) { documentSnapshot, error in
            if let error = error {
                ...
                showAlert(title: Err.userNotExistPleaaseSignUP, message: "", viewController: self)
            }
        }
    }
    ...
    func login(userName: String , phoneNumber: String) {
        ...
        PhoneAuthProvider.provider().verifyPhoneNumber("+91\(phoneNumber)", uiDelegate: nil) { [self] verificationID, error in
            ...
            loginButton.setTitle(Const.verify, for: .normal)
            loginButton.setTitle(Const.verify, for: .selected)
    ...
    func verifyCode(userName: String, mobile: String) {
        ...
        guard let code = codeTextField.text else {
            Utils.showToast(message: Err.pleaseEnterValidCode)
            return
        }
        ...
        Auth.auth().signIn(with: credential) { authResult, error in
            ...
            changeRequest.commitChanges { error in
                if let error = error {
                    ...
                    showAlert(title: Err.failedToLogin, message: "", viewController: self)
                } else {
    ...
    @objc private func handleShowSignUp() {
        navigationController?.pushViewController(SignUpController().self, animated: true)
    }
}
